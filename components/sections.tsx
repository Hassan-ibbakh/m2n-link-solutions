...
export function ProjectsGrid() {
  return <div className="mt-14 grid gap-6 lg:grid-cols-3">{projects.map((project) => <Reveal key={project.title}><Link href="/projects" className="group block"><div className={`project-visual ${project.accent}`}><span className="label absolute left-5 top-5 text-ink/50">{project.category}</span><div className="project-lines" /><span className="absolute bottom-5 left-5 font-display text-3xl">{project.title}</span><ArrowUpRight className="absolute bottom-5 right-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></div><div className="flex items-start justify-between gap-4 pt-5"><div><h3 className="font-display text-2xl">{project.title}</h3><p className="mt-2 text-sm leading-relaxed text-ink/60">{project.description}</p></div><span className="shrink-0 rounded-full bg-ink px-3 py-1.5 text-xs font-semibold text-paper">{project.result}</span></div></Link></Reveal>)}</div>;
}
...
export function Callout() {
  return <section className="bg-coral px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto flex max-w-7xl flex-col gap-10 lg:flex-row lg:items-end lg:justify-between"><Reveal><p className="label text-ink/60">Your next chapter</p><h2 className="mt-5 max-w-2xl font-display text-5xl leading-[.95] tracking-tight sm:text-7xl">Make the complicated useful.</h2></Reveal><ArrowLink href="/contact">Start a conversation</ArrowLink></div></section>;
}
